...
export default function Tags({ tags }: { tags: TagsCount }) {
  ...
  return (
    <>
      ...
      <h1 className="text-3xl font-extrabold leading-9 tracking-tight text-gray-900 dark:text-gray-100 sm:text-4xl sm:leading-10 text-center mt-20">
        All Tags
      </h1>
      <div className="flex flex-col items-start justify-start divide-y divide-gray-200 dark:divide-gray-700 md:mt-24 md:flex-row md:items-center md:justify-center md:space-x-6 md:divide-y-0">
        <div className="space-x-2 pb-8 pt-6 md:space-y-5"></div>
        <div className="flex max-w-lg flex-wrap">
          {Object.keys(tags).length === 0 && t('tag.no_tags_found')}
          {sortedTags.map((tag) => {
            return (
              <div key={tag} className="mb-2 mr-5 mt-1">
                <Tag text={tag} />
                <Link
